package com.lonely.nlp.algorithm;
...
public class Viterbi {
	...
	public static int[] compute(int []obs, int []states, double []start_p, double [][]trans_p, double [][]emit_p) {
		...
		for(int i=0; i<start_p.length; i++)
			System.out.print(start_p[i] + "\t");
		System.out.println();
				//print v
				System.out.println("********** trans:");
				for(int i=0; i<trans_p.length; i++) {
					for(int j=0; j<trans_p.length; j++)
						System.out.print(trans_p[j][i] + "\t");
					System.out.println();
				}
				//print path
		  		System.out.println("**********emit_p");
		  		for(int w=0; w<emit_p.length; w++) {
		  			for(int p=0; p<emit_p[0].length; p++) {
		  				System.out.print(emit_p[w][p] + "   ");
		  			}
		  			System.out.println();
		  		}
